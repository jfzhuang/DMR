...
class Transformer(nn.Module):
    def __init__(self, dim, depth, heads, mlp_dim):
        ...
        self.pe_enc = nn.Parameter(torch.rand([1, dim, 3, 64, 128]), requires_grad=True)
        self.pe_dec = nn.Parameter(torch.rand([1, dim, 1, 128, 256]), requires_grad=True) 
...
#         pe_enc = self.get_pe(x_enc, n, c, t_e, h, w)
        pe_enc = F.interpolate(self.pe_enc, size=(3, h, w), mode='trilinear')
...
#             pe_dec = pe_dec.repeat(n, 1, 1, 1, 1)
        ...
        x_enc = checkpoint.checkpoint(self.encoder, x_enc, pe_enc)
#         x_enc = self.encoder(x_enc, pos=pe_enc)
        
#         x_dec = checkpoint.checkpoint(self.decoder, x_dec, x_enc, pe_enc, pe_dec)        
        x_dec = self.decoder(x_dec, x_enc, pos=pe_enc, query_pos=self.pe_dec)
...
class STF(nn.Module):
    def __init__(self, dim, depth, heads, mlp_dim, pe_dim, use_checkpoint=False, block_size=[16, 16]):
        super().__init__()
    ...
    def forward(self, x_enc_0, x_enc_1, x_enc_2):
        ...
        if self.use_checkpoint:
            x_enc = checkpoint.checkpoint(self.encoder, x_enc, pe_enc)
#             x_dec = checkpoint.checkpoint(self.decoder, x_enc_1, x_enc, pe_enc, pe_dec)
        else:
            x_enc = self.encoder(x_enc, pos=pe_enc)
...
if __name__ == '__main__':
    ...
    model = STF(dim=512, depth=1, heads=2, mlp_dim=256, pe_dim=(128, 256), use_checkpoint=True)
    model.cuda().eval()
