...
from lib.net.transformer import CRM, ERM
...
class SegRefine(nn.Module):
    def __init__(self, num_classes=19):
        ...
        self.CRM = CRM(dim=512, depth=1, heads=2, mlp_dim=256, pe_dim=(128, 256))
        self.ERM = ERM(dim=512, mlp_dim=256, KL=10, KH=10, num_class=19)
        self.semantic_loss = nn.CrossEntropyLoss(ignore_index=255)

        self.weight_init()

    def weight_init(self):
        weight_path = '/data/jfzhuang/VSPW_480p/work_dirs_v2/ST_Memory/exp_new/cityscapes/psp50/CRM/best.pth'
        ...
        self.CRM.load_state_dict(CRM_weight, False)

        for p in self.ERM.parameters():
            if p.dim() > 1:
        ...
        self.ERM.memory_key.data = key.unsqueeze(0)
        ...
        self.ERM.memory_value.data = value.unsqueeze(0)
    ...
    def forward(self, im_list):
        with torch.no_grad():
            ...
            feat_1 = self.CRM(feat_0, feat_1, feat_2)
            
        feat_1 = self.ERM(feat_1)
