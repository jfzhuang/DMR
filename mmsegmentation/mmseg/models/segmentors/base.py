...
class BaseTemporalSegmentor(BaseSegmentor):
    def __init__(self, init_cfg=None):
        super(BaseTemporalSegmentor, self).__init__(init_cfg)

    @auto_fp16(apply_to=('clip',))
    def forward(self, clip, img_metas, return_loss=True, **kwargs):
        if return_loss:
            return self.forward_train(clip, img_metas, **kwargs)
        else:
            return self.forward_test(clip, img_metas, **kwargs)
